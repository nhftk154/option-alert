...
- זהב: GLD, IAU | כסף: SLV, SIVR (שני ה-ETF-ים הכי גדולים וסחירים לכל נכס)
- ביטקוין: IBIT, FBTC | את'ריום: ETHA, FETH (אותו עיקרון — ETF-ים רגילים עם
  אופציות רשומות, לא מסחר קריפטו ישיר)
...
- `scan.yml` רץ כל 10 דקות בשעות המסחר של NYSE, סורק שירד (1/6) מ-300 המניות
  בכל הרצה (כיסוי מלא כל שעה) + כל 8 ה-ETF-ים (זהב/כסף/ביטקוין/את'ריום)
  בכל הרצה. כולם נסרקים באותו pipeline בדיוק כמו מניה רגילה (yfinance).
...
- שילוב tvremix (`src/optionalert/tvremix_client.py`) נבדק מול מפתח אמיתי
  ועובד (אימות + פענוח תשובות תקינים). מוגבל ל-20 קריאות/דקה ו-200/שעה —
  ריצה שבועית של `refresh_universe.yml` על ~300 טיקרים יכולה למצות את
  המכסה השעתית לבד; זה בסדר, הרענון עוד מצליח לרוב הטיקרים, ומה שלא
  נפתר פשוט מדלג על שיפור ה-IV (לא מפיל שום דבר).
